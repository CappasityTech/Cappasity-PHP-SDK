...
class FileTest extends \PHPUnit\Framework\TestCase
{
    public function testTransformFile()
    {
        ...
        $this->assertEquals('ffd3edb7-cfbc-4880-8287-a6dc7ca84579', $file->getId());
        ...
        $fileItem = $files[0];
        $this->assertEquals(31347, $fileItem->getContentLength());
        $this->assertEquals('image/jpeg', $fileItem->getContentType());
        $this->assertEquals('BdqzIQNCTuqnrJWncVJRLg==', $fileItem->getMd5Hash());
        $this->assertEquals('cdn.cappasity3d.com', $fileItem->getBucket());
        $this->assertEquals('c-preview', $fileItem->getType());
        $this->assertEquals('c8b9c4579fc2c11a795785393c9aaf65/ffd3edb7-cfbc-4880-8287-a6dc7ca84579/7f0fdc79-9ef1-45fb-ac3f-6cabe6e0385c.jpeg', $fileItem->getFilename());
        $this->assertEquals(6, $attributes->getParts());
        $this->assertEquals('object', $attributes->getType());
        $this->assertEquals(1551090243029, $attributes->getUploadedAt());
        $embed = $attributes->getEmbed();
        $this->assertInstanceOf(Client\Model\Response\Files\Common\File\Attributes\Embed::class, $embed);
        $this->assertEquals('<iframe allowfullscreen mozallowfullscreen="true" webkitallowfullscreen="true" width="{{ width }}" height="{{ height }}" frameborder="0" style="border:0;" src="https://api.cappasity.com/api/player/ffd3edb7-cfbc-4880-8287-a6dc7ca84579/embedded?autorun={{ autorun }}&closebutton={{ closebutton }}&logo={{ logo }}&analytics={{ analytics }}&uipadx={{ uipadx }}&uipady={{ uipady }}&autorotate={{ autorotate }}&autorotatetime={{ autorotatetime }}&autorotatedelay={{ autorotatedelay }}&autorotatedir={{ autorotatedir }}&hidefullscreen={{ hidefullscreen }}&hideautorotateopt={{ hideautorotateopt }}&hidesettingsbtn={{ hidesettingsbtn }}&enableimagezoom={{ enableimagezoom }}&zoomquality={{ zoomquality }}&hidezoomopt={{ hidezoomopt }}"></iframe>', $embed->getCode());
        $params = $embed->getParams();
        $this->assertInstanceOf(Client\Model\Response\Files\Common\File\Attributes\Embed\Params::class, $params);
        $this->assertEquals('boolean', $params->getAutoRun()->getType());
        $this->assertEquals(1, $params->getCloseButton()->getDefault());
        $this->assertEquals(1, $params->getLogo()->getDefault());
        $this->assertEquals('Enable analytics', $params->getAnalytics()->getDescription());
        ...
        $this->assertEquals('cdn.cappasity3d.com', $attributes->getBucket());
        ...
        $this->assertEquals('cappasity', $attributes->getOwner());
    }
}
